...
```rs
fn call(agent: Context<'_, '_, '_>, value: Value) -> Value {
    // ...
}
```
...
```rs
fn call(agent: Context<'_, '_, '_>, value: Register<Value<'_>>) -> Register<Value<'_>> {
...
```rs
fn call(agent: Context<'_, '_, '_>, value: Register<Value<'_>>) -> JsResult<Register<Value<'_>>> {
    let value = unsafe { value.bind(agent) };
